...
//     doc.save("data-all-proposal.pdf");
//   };
...
const AllProposalAdmin = () => {
  ...
  // Variabel state untuk pencarian
  const [searchTerm, setSearchTerm] = useState("");
  const [loading, setLoading] = useState(true);
  ...
  useEffect(() => {
    ...
    const filtered = proposals.filter((proposal) => {
      ...
      return (
        proposal.publication_title?.toLowerCase().includes(searchTermLower) ||
        proposal.lecturer?.name?.toLowerCase().includes(searchTermLower) ||
        proposal.status?.status_name?.toLowerCase().includes(searchTermLower) ||
        formatDate(proposal.createdAt)
          .toLowerCase()
          .includes(searchTermLower) ||
        formattedDate.toLowerCase().includes(searchTermLower) ||
        proposal.id?.toString().includes(searchTermLower)
      );
    });
    setFilteredProposals(filtered);
    setCurrentPage(1);
