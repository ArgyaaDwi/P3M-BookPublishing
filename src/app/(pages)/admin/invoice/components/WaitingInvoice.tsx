...
const WaitingInvoiceAdmin = () => {
  ...
  const handleExportPDF = () => {
    ...
    exportToPDF({
      head: headers,
      body: body,
      filename: `data-pending-invoice-halaman-${currentPage}`,
    });
  };
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(data, "semua-invoice-pending");
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch("/api/v1/admin/invoices?status=pending");
...
export default WaitingInvoiceAdmin;
