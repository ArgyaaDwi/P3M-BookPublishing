...
type ModalRevisionFinalDocProps = {
  proposal: {
    id: number;
...
const ModalRevisionFinalDoc: React.FC<ModalRevisionFinalDocProps> = ({
  proposal,
}) => {
  ...
  useEffect(() => {
    if (proposal) {
      setFinalBookUrl(proposal.publication_final_book || "");
    }
  }, [proposal, isOpen]);
    ...
    try {
      ...
      if (res.ok) {
        await Swal.fire({
          icon: "success",
          title: "Berhasil!",
          text: "Berhasil revisi berkas final!",
        ...
        await Swal.fire({
          icon: "error",
          title: "Gagal!",
          text: "Gagal mengupload revisi berkas final: " + result.message,
  ...
  return (
    <div>
      <button
        className="bg-amber-100 p-2 rounded-lg text-amber-600 hover:text-amber-800 flex items-center gap-2"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl p-8">
            <h3 className="text-2xl font-semibold text-gray-900 text-center mb-4">
              Perbaikan Dokumen Final Ajuan Buku
            </h3>
...
export default ModalRevisionFinalDoc;
