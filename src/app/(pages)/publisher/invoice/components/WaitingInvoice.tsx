...
const WaitingInvoicePublisher = () => {
  ...
  const handleExportExcel = () => {
    ...
    exportToExcel(data, "verify-invoice");
  };
  useEffect(() => {
    if (!searchTerm.trim()) {
  ...
  const fetchData = async () => {
    try {
      const res = await fetch("/api/v1/publisher/invoice?status=pending");
...
export default WaitingInvoicePublisher;
